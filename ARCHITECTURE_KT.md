...
**KT Note - Keeping Users in the Loop:**
Even with real-time WebSockets, users might close the browser while waiting for the AI to process or an Admin to reply. To solve this, we implemented an automated email notification system.

### Step 39: The Email Service Integration (`EmailService.java`)
We integrated `spring-boot-starter-mail` to handle SMTP communications. Instead of relying on a local mock SMTP server (like MailHog) which cannot send real emails to the outside world, we configured Spring Boot to connect directly to **Gmail's SMTP servers** (`smtp.gmail.com`). 

### Step 40: App Passwords & Security
To prevent Google from blocking the connection and to avoid hardcoding raw personal passwords into the codebase, we utilize **Google App Passwords**. This allows our Java backend to securely authenticate as an authorized device. The credentials (`SPRING_MAIL_USERNAME` and `SPRING_MAIL_PASSWORD`) are dynamically injected via environment variables from the `.env` file into our `docker-compose.yml`, ensuring secrets never touch source control.

### Step 41: Rich HTML Formatting
Standard text emails strip out the rich Markdown formatting generated by the Gemini AI. To provide a premium user experience, we generate **MIME (Multipurpose Internet Mail Extensions)** messages. The `EmailService` intercepts the AI's raw Markdown response, uses Regex to convert syntax like `**bold**` and `\n` into rich HTML tags (`<b><i>` and `<br>`), and injects a custom, styled HTML footer signature before dispatching the email via `JavaMailSender`.
